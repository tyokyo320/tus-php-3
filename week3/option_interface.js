...
function sentakushi(c, choice, name){
    ...
    input.name = name;
    input.value = c;
    input.id = 'choice' + (c + 1);  // 追加
    input.classList.add("custom_control-input");    // 追加

    let label = document.createElement("label");
    label.setAttribute("for", "choice" + (c + 1));  // 追加
    label.innerHTML = choice;
    label.classList.add("custom_control-label");    // 追加
    
    let div = document.createElement("div");
    div.classList.add("custom_control");
    div.classList.add("custom_radio");
